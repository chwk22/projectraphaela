...
const STORAGE_PREFIX = 'raphaela:v1:';
...
.rp-sub { color: var(--muted); font-size: 13.5px; margin: 6px 0 0; max-width: 46ch; }

/* ---- Date stepper ---- */
.rp-datebar { display: flex; align-items: center; gap: 8px; }
.rp-date {
...
.rp-btn:hover { background: #232a52; border-color: #3b4680; }
.rp-btn:disabled { opacity: 0.45; cursor: not-allowed; }
.rp-btn--icon { padding: 7px 8px; }
.rp-btn--primary {
...
.rp-btn--primary:hover { background: #5b8d74; border-color: #5b8d74; }
.rp-btn--done { background: var(--eucalypt); border-color: var(--eucalypt); color: #0D2A1B; }
.rp-btn--done:hover { background: var(--eucalypt); border-color: var(--eucalypt); }

/* ---- Segmented control ---- */
.rp-seg { display: inline-flex; border: 1px solid var(--hair); border-radius: 3px; overflow: hidden; }
.rp-seg button {
  font: inherit; font-size: 13px; font-weight: 500;
  display: inline-flex; align-items: center; gap: 7px;
...
.rp-seg button + button { border-left: 1px solid var(--hair); }
.rp-seg button:hover { color: var(--paper); }
.rp-seg button[aria-pressed='true'] { background: var(--ink-2); color: var(--paper); }

/* ---- Layout ---- */
.rp-grid { display: grid; grid-template-columns: minmax(0, 1.6fr) minmax(310px, 1fr); gap: 26px; align-items: start; }
@media (max-width: 940px) { .rp-grid { grid-template-columns: 1fr; gap: 22px; } }
...
.rp-mapratio { width: 100%; height: 0; }
.rp-maplayer { position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: block; }
.rp-mapcanvas { image-rendering: auto; pointer-events: none; }
.rp-mapsvg { cursor: crosshair; touch-action: manipulation; }
.rp-mapsvg:focus-visible { outline: 2px solid var(--eucalypt); outline-offset: -2px; }

.rp-land { fill: #171D3B; stroke: #39457E; stroke-width: 1.6; stroke-linejoin: round; }
.rp-landtop { fill: none; stroke: rgba(147,195,168,0.34); stroke-width: 1; }

.rp-spotdot { fill: rgba(234,233,244,0.5); }
.rp-spotdot--top { fill: var(--eucalypt); }
.rp-spothit { fill: transparent; cursor: pointer; }
.rp-spothit:hover + .rp-spotdot, .rp-spothit:focus-visible + .rp-spotdot { fill: #fff; }
...
.rp-legendramp { width: 88px; height: 8px; border-radius: 1px; }

/* ---- Transport controls ---- */
.rp-transport { display: flex; align-items: center; gap: 12px; margin-top: 12px; flex-wrap: wrap; }
.rp-clock {
  font-family: var(--display); font-size: 25px; font-weight: 600;
  letter-spacing: -0.01em; min-width: 78px;
}
.rp-clocknote { font-size: 11.5px; color: var(--faint); margin-top: -2px; }

.rp-range { flex: 1 1 200px; min-width: 140px; }
.rp-range input[type='range'] {
  -webkit-appearance: none; appearance: none;
  width: 100%; height: 22px; background: transparent; cursor: pointer; margin: 0; display: block;
}
.rp-range input[type='range']::-webkit-slider-runnable-track {
  height: 3px; background: var(--hair); border-radius: 2px;
}
.rp-range input[type='range']::-moz-range-track {
  height: 3px; background: var(--hair); border-radius: 2px;
}
.rp-range input[type='range']::-webkit-slider-thumb {
  -webkit-appearance: none; appearance: none;
  width: 15px; height: 15px; border-radius: 50%;
  background: var(--paper); border: 0; margin-top: -6px;
}
.rp-range input[type='range']::-moz-range-thumb {
  width: 15px; height: 15px; border-radius: 50%;
  background: var(--paper); border: 0;
}

/* ---- Day ribbon ---- */
.rp-ribbonwrap { margin-top: 16px; }
.rp-ribbon { display: block; width: 100%; height: 76px; }

/* ---- Right rail ---- */
.rp-panel { border: 1px solid var(--hair); border-radius: 3px; padding: 18px; background: var(--ink-1); }
.rp-panel + .rp-panel { margin-top: 18px; }

.rp-eyebrow { font-size: 12px; color: var(--faint); margin: 0 0 3px; }
.rp-place { font-family: var(--display); font-size: 23px; font-weight: 600; line-height: 1.15; margin: 0; }
.rp-coords { font-size: 12px; color: var(--muted); margin: 5px 0 0; }
.rp-note { font-size: 13px; color: var(--muted); margin: 12px 0 0; }

.rp-hero { margin: 18px 0 0; padding: 14px 0; border-top: 1px solid var(--hair); border-bottom: 1px solid var(--hair); }
.rp-herolabel { font-size: 12.5px; color: var(--muted); margin: 0 0 2px; }
.rp-herotime {
  font-family: var(--display); font-size: 46px; font-weight: 600;
  line-height: 1; letter-spacing: -0.02em; color: var(--dawn); margin: 0;
}
.rp-herozone { font-family: var(--ui); font-size: 14px; font-weight: 500; color: var(--muted); margin-left: 8px; letter-spacing: 0; }
.rp-herohint { font-size: 12.5px; color: var(--faint); margin: 7px 0 0; }

.rp-rows { margin: 14px 0 0; }
.rp-row {
  display: flex; align-items: baseline; justify-content: space-between; gap: 12px;
  padding: 7px 0; font-size: 13.5px;
}
.rp-row + .rp-row { border-top: 1px solid var(--hair-soft); }
.rp-row dt { color: var(--muted); margin: 0; }
.rp-row dd { margin: 0; font-weight: 500; font-variant-numeric: tabular-nums; }

/* ---- Score ---- */
.rp-scorehead { display: flex; align-items: baseline; justify-content: space-between; gap: 10px; }
.rp-scorenum { font-family: var(--display); font-size: 30px; font-weight: 600; line-height: 1; }
.rp-scoregrade { font-size: 13px; color: var(--eucalypt); }
.rp-bars { margin-top: 12px; }
.rp-bar { display: grid; grid-template-columns: 74px 1fr 30px; align-items: center; gap: 9px; font-size: 12px; padding: 3px 0; }
.rp-bar span:first-child { color: var(--muted); }
.rp-bartrack { height: 4px; background: var(--ink-2); border-radius: 2px; overflow: hidden; }
.rp-barfill { height: 100%; background: var(--eucalypt-deep); border-radius: 2px; }
.rp-bar span:last-child { color: var(--muted); text-align: right; font-variant-numeric: tabular-nums; }

/* ---- Ranked list ---- */
.rp-rank { width: 100%; border-collapse: collapse; font-size: 13.5px; }
.rp-rank th {
  text-align: left; font-size: 12px; font-weight: 500; color: var(--faint);
  padding: 0 10px 8px 0; border-bottom: 1px solid var(--hair);
}
.rp-rank th:last-child, .rp-rank td:last-child { text-align: right; padding-right: 0; }
.rp-rank td { padding: 9px 10px 9px 0; border-bottom: 1px solid var(--hair-soft); }
.rp-rank tbody tr { cursor: pointer; }
.rp-rank tbody tr:hover td { background: var(--ink-1); }
.rp-rank tbody tr[aria-selected='true'] td { background: var(--ink-2); }
.rp-rankpos { color: var(--faint); width: 26px; font-variant-numeric: tabular-nums; }
.rp-rankname { font-weight: 500; }
.rp-rankregion { color: var(--faint); font-size: 12px; margin-left: 7px; font-weight: 400; }
.rp-rankscore { font-variant-numeric: tabular-nums; }

/* ---- Saved list ---- */
.rp-saved { display: flex; align-items: center; gap: 10px; padding: 9px 0; font-size: 13.5px; }
.rp-saved + .rp-saved { border-top: 1px solid var(--hair-soft); }
.rp-savedmain { flex: 1; min-width: 0; text-align: left; background: none; border: 0; color: inherit; font: inherit; cursor: pointer; padding: 0; }
.rp-savedmain:hover { color: var(--eucalypt); }
.rp-savedname { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.rp-savedmeta { display: block; font-size: 11.5px; color: var(--faint); }
.rp-iconbtn {
  background: none; border: 0; color: var(--faint); cursor: pointer;
  padding: 4px; display: inline-flex; border-radius: 2px;
}
.rp-iconbtn:hover { color: var(--ember); }

/* ---- Forms ---- */
.rp-field { display: block; margin-top: 12px; font-size: 12.5px; color: var(--muted); }
...
.rp-fieldrow { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
.rp-err { color: var(--ember); font-size: 12px; margin-top: 6px; display: flex; gap: 6px; align-items: flex-start; }
...
.rp-notice svg { flex: none; margin-top: 1px; color: var(--dawn); }

.rp-foot { margin-top: 26px; padding-top: 16px; border-top: 1px solid var(--hair); font-size: 12px; color: var(--faint); max-width: 78ch; }
.rp-foot p { margin: 0 0 7px; }
...
.rp-toast--bad { border-left: 2px solid var(--ember); }
.rp-toast--good { border-left: 2px solid var(--eucalypt); }
.rp-toast textarea {
...
/* ---- Error boundary ---- */
.rp-crash { max-width: 60ch; margin: 60px auto; padding: 26px; border: 1px solid var(--hair); border-radius: 3px; }
.rp-crash h2 { font-family: var(--display); font-size: 24px; font-weight: 600; margin: 0 0 10px; }
.rp-crash pre {
...
@media (max-width: 560px) {
  ...
  .rp-herotime { font-size: 38px; }
  .rp-head { gap: 12px; }
  .rp-datebar { width: 100%; }
  .rp-date { flex: 1; min-width: 0; }
}
`;
...
class ErrorBoundary extends Component {
  ...
  componentDidCatch(error, info) {
    ...
    console.error('RaphaelaProject crashed:', error, info);
  }
  ...
  return (
    <div>
      <div className="rp-mapwrap">
        ...
        <div className="rp-mapratio" style={{ paddingTop: `${(VB_H / VB_W) * 100}%` }} />

        <canvas
          ref={canvasRef}
          className="rp-maplayer rp-mapcanvas"
          width={GRID_W}
          height={GRID_H}
          aria-hidden="true"
        />
        ...
        <svg
          ref={svgRef}
          className="rp-maplayer rp-mapsvg"
        ...
        >
          ...
          {LAND_PATHS.map((d, i) => (
            <path key={`land-${i}`} d={d} className="rp-land" />
          ))}
          {LAND_PATHS.map((d, i) => (
            <path key={`rim-${i}`} d={d} className="rp-landtop" />
          ))}
          ...
          {SPOTS.map((spot) => (
            <g key={spot.id}>
              ...
              />
              <circle
                className={`rp-spotdot${topFive.has(spot.id) ? ' rp-spotdot--top' : ''}`}
                cx={projectX(spot.lon)}
                cy={projectY(spot.lat)}
  ...
  return (
    <div className="rp">
      ...
      <header className="rp-head">
        <div>
          <h1 className="rp-title">RaphaelaProject</h1>
          <p className="rp-sub">
        ...
        <div className="rp-datebar">
          ...
          <div className="rp-seg" role="group" aria-label="Choose sunrise or sunset">
            <button
              type="button"
              aria-pressed={isRise}
      ...
      {dateError && (
        <div className="rp-notice" role="alert">
      ...
      <div className="rp-grid">
        ...
        <div>
          ...
          <div className="rp-transport">
            ...
            <div>
              <div className="rp-clock">{formatInZone(mapTime, timeZone)}</div>
              <div className="rp-clocknote">
          ...
          {/* ---------------- Ranking ---------------- */}
          <div className="rp-panel" style={{ marginTop: 20 }}>
            ...
            <h2 className="rp-place" style={{ marginBottom: 14 }}>Where the light should be best</h2>

            <table className="rp-rank">
              <thead>
                <tr>
                  <th aria-label="Rank" />
                  <th>Place</th>
              ...
              <tbody>
                {ranked.slice(0, 10).map((row, i) => (
                  ...
                  >
                    <td className="rp-rankpos">{i + 1}</td>
                    <td>
                      <span className="rp-rankname">{row.spot.name}</span>
                      <span className="rp-rankregion">{row.spot.region}</span>
                    </td>
                    <td className="rp-rankscore">
                    ...
                    <td className="rp-rankscore">{row.score.total}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div>
          <div className="rp-panel">
            <p className="rp-eyebrow">{site.region}</p>
            <h2 className="rp-place">{site.name}</h2>
            <p className="rp-coords">
              {site.lat.toFixed(4)}, {site.lon.toFixed(4)} · {timeZone.split('/')[1]}
            </p>

            {offMap && (
              <div className="rp-notice" style={{ marginTop: 12, marginBottom: 0 }}>
                <AlertTriangle size={15} />
            ...
            {times && times.regime !== 'normal' ? (
              ...
              <>
                <div className="rp-hero">
                  <p className="rp-herolabel">Peak colour</p>
                  <p className="rp-herotime">
                    {formatInZone(peakTime, timeZone)}
                    <span className="rp-herozone">{zone}</span>
                  </p>
                  <p className="rp-herohint">
                ...
                <div style={{ marginTop: 16 }}>
                  <button
                    className={`rp-btn rp-btn--primary${copied ? ' rp-btn--done' : ''}`}
                    onClick={handleCopy}
                  >
            ...
            {site.note && <p className="rp-note">{site.note}</p>}
          </div>

          {/* ---- Score ---- */}
          {score && times && times.regime === 'normal' && (
            <div className="rp-panel">
              <div className="rp-scorehead">
                <div>
                  <p className="rp-eyebrow">Light score</p>
                  <p className="rp-scorenum">
                    {score.total}
                    <span style={{ fontSize: 15, color: 'var(--faint)' }}> /100</span>
                  </p>
                </div>
                <span className="rp-scoregrade">{score.grade}</span>
              </div>
              ...
              <div className="rp-bars">
                {[
                  ...
                  <div className="rp-bar" key={label}>
                    <span>{label}</span>
                    <span className="rp-bartrack">
                      <span className="rp-barfill" style={{ width: `${(value / max) * 100}%` }} />
                    </span>
                    <span>{value}</span>
                  </div>
                ))}
              ...
              <p className="rp-note">{score.headline}</p>
            </div>
          )}
          ...
          {site.isCustom && (
            <div className="rp-panel">
              <p className="rp-eyebrow">Refine this pin</p>
              <h3 className="rp-place" style={{ fontSize: 18 }}>What can you see from here?</h3>
              <p className="rp-note" style={{ marginTop: 6 }}>
          ...
          {/* ---- Typed coordinates ---- */}
          <div className="rp-panel">
            <p className="rp-eyebrow">Somewhere specific</p>
            <h3 className="rp-place" style={{ fontSize: 18 }}>Type a coordinate</h3>

            <div className="rp-fieldrow">
              <label className="rp-field">
                Latitude
                <input
                  type="text"
          ...
          {/* ---- Peak offset ---- */}
          <div className="rp-panel">
            <p className="rp-eyebrow">Peak colour rule</p>
            <label className="rp-field" style={{ marginTop: 8 }}>
          ...
          {/* ---- Saved ---- */}
          <div className="rp-panel">
            <p className="rp-eyebrow">On this device</p>
            <h3 className="rp-place" style={{ fontSize: 18, marginBottom: 6 }}>Saved spots</h3>
            ...
            {saved.length === 0 ? (
              ...
              <div>
                {saved.map((entry) => (
                  <div className="rp-saved" key={entry.key}>
                    <button
                      className="rp-savedmain"
                      onClick={() =>
                    ...
                    >
                      <span className="rp-savedname">{entry.name}</span>
                      <span className="rp-savedmeta">
                        {entry.lat.toFixed(3)}, {entry.lon.toFixed(3)}
                      </span>
                    </button>
                    <button
                      className="rp-iconbtn"
      ...
      {toast && (
        <div className={`rp-toast rp-toast--${toast.kind === 'bad' ? 'bad' : 'good'}`} role="status">
          <div style={{ flex: 1 }}>
            {toast.message}
            {toast.text && (
          ...
          <button className="rp-iconbtn" onClick={() => setToast(null)} aria-label="Dismiss">
            <X size={15} />
          </button>
        </div>
      )}
 ...
 */
export default function RaphaelaProject() {
  return (
    <ErrorBoundary>
      <RaphaelaProjectInner />
    </ErrorBoundary>
  );
}
